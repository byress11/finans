...
const ReceiptScanner = {
    ...
    // Tutar çıkar
    extractAmount(text) {
        // Türkçe para formatları
        const patterns = [
            // 1.234,56 TL veya ₺1.234,56
            /(?:toplam|tutar|total|genel\s*toplam|ödenecek|ödenen|nakit|kart)[\s:]*[₺]?\s*([\d.,]+)\s*(?:tl|₺)?/gi,
            /[₺]\s*([\d.,]+)/g,
            /([\d.,]+)\s*(?:tl|₺)/gi,
            // Sadece rakamlar (virgül veya nokta ile)
            /(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2}))/g
        ];
        ...
        for (const pattern of patterns) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                // Türkçe formatı düzelt (1.234,56 -> 1234.56)
                if (amountStr.includes(',')) {
                    // Son virgülden sonrası kuruş
                    const parts = amountStr.split(',');
                    const lastPart = parts[parts.length - 1];
                    
                    if (lastPart.length === 2) {
                        // 1.234,56 formatı
                        amountStr = parts.slice(0, -1).join('').replace(/\./g, '') + '.' + lastPart;
                    } else {
                        // 1,234.56 formatı olabilir
        ...
        if (amounts.length > 0) {
            // Tutarları sırala ve en büyüğünü al
