...
const FirebaseSync = {
        ...
        try {
            ...
            this.auth.onAuthStateChanged(async user => {
                ...
                if (user) {
                    ...
                    await this.forceReplaceFromCloud({ silent: true });
    ...
    async signIn(email, password) {
        try {
            ...
            await this.forceReplaceFromCloud();
            return result.user;
        } catch (error) {
    ...
    async syncToCloud(options = {}) {
        ...
        try {
            ...
            for (const profile of AppState.profiles) {
                batch.set(profilesRef.doc(profile.id), profile);
            }
    ...
    async forceUploadToCloud(options = {}) {
        ...
        try {
            ...
            // First, clear all cloud data
            await this.clearCloudData({ silent: true });

            // Then upload all local data to cloud
            const batch = this.db.batch();
